...
class EditCategory extends StatefulWidget {
  Category category;
  User user;
  EditCategory({super.key, required this.category, required this.user});
...
class _EditCategoryState extends State<EditCategory> {
  ...
  @override
  void initState() {
    money = this.widget.category.money.toString();
    oldmoney = this.widget.category.money;
    _selectedItem = this.widget.category.category;
    _typeMoney = this.widget.category.type;
    noteController.text = this.widget.category.note;
    selectedDate = this.widget.category.time;
    if (this.widget.category.image) {
      getImageUrl(this.widget.category.id);
    }
    updateMoney(money!);
    setState(() {});
    super.initState();
  ...
  Future<void> getImageUrl(String id) async {
    ...
    //     await FirebaseFirestore.instance.collection('images').doc('$id.png');
    final storageRef = FirebaseStorage.instance.ref();
    final imageUrl = await storageRef.child("images/$id.png").getDownloadURL();
    setState(() {
      UrlImage = imageUrl;
    });
  }
  ...
  UpdateCategory({
    ...
    final docCategories = FirebaseFirestore.instance
        .collection('category')
        .doc("${this.widget.category.id}");
    docCategories.update(category.toJson());
    int _updateMoney = 0;
    if (type == "Chi Tiêu Tăng") {
      print(money);
      print(this.widget.user.accountBalance);
      _updateMoney = this.widget.user.accountBalance - money;
      print(_updateMoney);
      docUsers.update({'accountBalance': _updateMoney});
    } else if (type == "Chi Tiêu Giảm") {
      print(money);
      print(this.widget.user.accountBalance);
      _updateMoney = this.widget.user.accountBalance + money;
      print(_updateMoney);
      docUsers.update({'accountBalance': _updateMoney});
    } else if (type == "Thu Nhập Giảm") {
      _updateMoney = this.widget.user.accountBalance - money;
      print(_updateMoney);
      docUsers.update({'accountBalance': _updateMoney});
    } else if (type == "Thu Nhập Tăng") {
      _updateMoney = this.widget.user.accountBalance + money;
      print(_updateMoney);
      docUsers.update({'accountBalance': _updateMoney});
    }
                                ...
                                child: TextButton(
                                  onPressed: () {
                                    ...
                                    if (_selectedItem == "") {
                                      ...
                                      if (imageFile != null || UrlImage != "") {
                                        if (imageFile != null)
                                          uploadFile(imageFile!,
                                              this.widget.category.id);
                                        int newMoney = int.parse(
                                            showMoney!.replaceAll(",", ""));
                                        String type = "";
                                        if (_typeMoney == "Chi Tiêu") {
                                          if (newMoney > oldmoney) {
                                            //nếu số tiền thêm mới lớn hơn số tiền cũ thì tiền mới sẽ bằng tiền mới từ tiền cũ sau đó lấy tiền mới trừ lại vào tài khoản
                                            newMoney = newMoney - oldmoney;
                                            type = "Chi Tiêu Tăng";
                                          } else if (newMoney < oldmoney) {
                                            //nếu số tiền mới nhỏ hơn số tiền cũ thì tiền mới sẽ bằng tiền cũ trừ cho tiền mới sau đó cộng lại vào tài khoản số tiền chênh lệch đó
                                            newMoney = oldmoney - newMoney;
                                            type = "Chi Tiêu Giảm";
                                          } else if (newMoney == oldmoney) {
                                            type = "Không Đổi";
                                          }
                                        } else if (_typeMoney == "Thu Nhập") {
                                          if (newMoney > oldmoney) {
                                            // ngược lại cộng thêm lại vào số dư
                                            type = "Thu Nhập Giảm";
                                            newMoney = newMoney - oldmoney;
                                          } else if (newMoney < oldmoney) {
                                            // ngược lại trừ vào số dư
                                            type = "Thu Nhập Tăng";
                                            newMoney = oldmoney - newMoney;
                                          } else if (newMoney == oldmoney) {
                                            type = "Không Đổi";
